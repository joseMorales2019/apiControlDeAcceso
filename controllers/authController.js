...
//     if (!tenantId) {
//       return res.status(400).json({ message: 'tenantId es obligatorio para iniciar sesión' });
//     }
...
export const login = async (req, res) => {
  try {
    ...
    if (!email && !documento) {
      return res.status(400).json({ message: 'Debe proporcionar email o documento para iniciar sesión' });
    }
...
// El resto del archivo (profile, getUsers, updateUser, etc.) no requiere cambio para este caso
// Puedes mantener el contenido existente sin necesidad de modificarlo
